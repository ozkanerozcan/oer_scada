...
export default function Press500TPres1() {
  ...
  // Device status-based online detection (from backend device:status messages)
  const pres1Device = devices.find(d => d.name === 'Pres1')
  const pres1DeviceId = pres1Device?.id || 5
  const deviceStatus = statuses[pres1DeviceId]
  
  // Tag-based online detection (5-second timeout) - as fallback
  const getLatestTagTimestamp = () => {
    const tagKeys = Object.keys(values).filter(key => key.startsWith('Pres1.Press_500T.'))
    if (tagKeys.length === 0) return 0
    return Math.max(...tagKeys.map(key => values[key]?.timestamp || 0))
  }

  const TIMEOUT_MS = 5000
  const latestTimestamp = getLatestTagTimestamp()
  const isTagOnline = (Date.now() - latestTimestamp) < TIMEOUT_MS
  
  // Use device status from backend if available, otherwise fallback to tag timestamp
  const isPRes1Online = deviceStatus === 'connected' ? true : 
  ...
  return (
    <>
      ...
      <div style={{ padding: paddingSize, height: '100%', display: 'flex', flexDirection: 'column', fontFamily: "'Inter', sans-serif" }}>
        ...
        <div className="card" style={{ padding: cardPadding, borderRadius: 16, display: 'flex', gap: 12, alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        <div className="card" style={{ padding: cardPadding, borderRadius: 16, marginBottom: 24 }}>
